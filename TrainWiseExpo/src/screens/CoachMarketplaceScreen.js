...
// First-visit walkthrough for the Coach Marketplace screen (shown once, tracked
// by tutorialManager under the 'coachMarketplace' key).
...
const CoachMarketplaceScreen = ({ navigation, route }) => {
  ...
  useEffect(() => {
    isTutorialDone('coachMarketplace')
      .then((done) => { if (!done) setShowTutorial(true); })
      .catch((e) => console.warn('[CoachMarketplaceScreen] tutorial check failed:', e.message));
  }, []);

  const handleTutorialFinish = async () => {
    await markTutorialDone('coachMarketplace');
