...
// First-visit walkthrough for the Program Builder screen (shown once,
// tracked by tutorialManager under the 'programBuilder' key).
...
const ProgramBuilderScreen = ({ route, navigation }) => {
  ...
  useEffect(() => {
    ...
    isTutorialDone('programBuilder')
      .then((done) => { if (!done) setShowTutorial(true); })
      .catch((e) => console.warn('[ProgramBuilderScreen] tutorial check failed:', e.message));
  }, []);

  const handleTutorialFinish = async () => {
    await markTutorialDone('programBuilder');
