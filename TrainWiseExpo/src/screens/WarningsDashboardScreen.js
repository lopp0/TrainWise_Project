...
// First-visit walkthrough for the Warnings dashboard (shown once, tracked by
// tutorialManager under the 'warnings' key).
...
const WarningsDashboardScreen = () => {
  ...
  const [showTutorial, setShowTutorial] = useState(false); // first-visit walkthrough
  ...
  useEffect(() => {
    isTutorialDone('warnings').then((done) => {
      if (!done) setShowTutorial(true);
    });
  }, []);

  const handleTutorialFinish = async () => {
    await markTutorialDone('warnings');
