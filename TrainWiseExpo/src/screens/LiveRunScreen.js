...
const paceStr = (km, sec) => {
  ...
  const perKm = sec / 60 / km;
  const m = Math.floor(perKm);
  const s = Math.round((perKm - m) * 60);
  return `${m}:${String(s === 60 ? 0 : s).padStart(2, '0')}/km`;
};

// First-visit walkthrough for the Live Run screen (shown once, tracked by
// tutorialManager under the 'liveRun' key).
...
const LiveRunScreen = ({ navigation, route }) => {
  ...
  useEffect(() => {
    ...
    isTutorialDone('liveRun')
      .then((done) => { if (!done) setShowTutorial(true); })
      .catch((e) => console.warn('[LiveRunScreen] tutorial check failed:', e.message));
  }, []);

  const handleTutorialFinish = async () => {
    await markTutorialDone('liveRun');
