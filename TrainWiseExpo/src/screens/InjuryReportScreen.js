...
// First-visit walkthrough for the Injury Report screen (shown once, tracked
// by tutorialManager under the 'injuryReport' key).
...
const InjuryReportScreen = ({ navigation, route }) => {
  ...
  // First-visit tutorial (shows once under the 'injuryReport' key).
  ...
  useEffect(() => {
    isTutorialDone('injuryReport').then((done) => {
      if (!done) setShowTutorial(true);
    });
  }, []);

  const handleTutorialFinish = async () => {
    await markTutorialDone('injuryReport');
