...
// First-visit walkthrough for the Challenges screen (shown once, tracked by
// tutorialManager under the 'challenges' key).
...
const ChallengesScreen = ({ navigation }) => {
  ...
  React.useEffect(() => {
    isTutorialDone('challenges')
      .then((done) => { if (!done) setShowTutorial(true); })
      .catch((e) => console.warn('[ChallengesScreen] tutorial check failed:', e.message));
  }, []);

  const handleTutorialFinish = async () => {
    await markTutorialDone('challenges');
