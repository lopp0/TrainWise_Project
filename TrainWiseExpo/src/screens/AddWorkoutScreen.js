...
// First-visit walkthrough for the Add Workout screen (shown once, tracked by
// tutorialManager under the 'addWorkout' key).
...
const AddWorkoutScreen = ({ navigation, route }) => {
  ...
  // First-visit tutorial (shows once under the 'addWorkout' key).
  useEffect(() => {
    isTutorialDone('addWorkout').then((done) => {
      if (!done) setShowTutorial(true);
    });
  }, []);

  const handleTutorialFinish = async () => {
    await markTutorialDone('addWorkout');
